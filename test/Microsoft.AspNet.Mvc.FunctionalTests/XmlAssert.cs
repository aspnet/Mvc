...
namespace Microsoft.AspNet.Mvc.FunctionalTests
{
    ...
    public static class XmlAssert
    {
        ...
        {
            var sortedExpectedXDoc = Sort(XDocument.Parse(expectedXml));
            var sortedActualXDoc = Sort(XDocument.Parse(actualXml));

            bool areEqual = XNode.DeepEquals(sortedExpectedXDoc, sortedActualXDoc);

            if (!areEqual)
            {
                throw new EqualException(expectedXml, actualXml);
            }
        }

        private static XDocument Sort(XDocument doc)
        {
