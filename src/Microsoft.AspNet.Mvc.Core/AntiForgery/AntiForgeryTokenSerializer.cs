...
namespace Microsoft.AspNet.Mvc
{
    internal sealed class AntiForgeryTokenSerializer : IAntiForgeryTokenSerializer
    {
        ...
        public AntiForgeryToken Deserialize(string serializedToken)
        {
            ...
            try
            {
                var tokenBytes = AntiForgeryTokenEncodingHelper.UrlTokenDecode(serializedToken);
        ...
        public string Serialize([NotNull] AntiForgeryToken token)
        {
            using (var stream = new MemoryStream())
            {
                using (var writer = new BinaryWriter(stream))
                {
                    ...
                    writer.Flush();
                    return AntiForgeryTokenEncodingHelper.UrlTokenEncode(_cryptoSystem.Protect(stream.ToArray()));
                }
            }
        }
    }
