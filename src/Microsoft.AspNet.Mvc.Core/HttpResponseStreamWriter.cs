...
namespace Microsoft.AspNet.Mvc
{
    ...
    public class HttpResponseStreamWriter : TextWriter
    {
            ...
            ArrayPool<char> charPool)
        {
            ...
            try
            {
                _charBuffer = charPool.Rent(_charBufferSize);

                var requiredLength = encoding.GetMaxByteCount(bufferSize) - encoding.GetPreamble().Length;
                _byteBuffer = bytePool.Rent(requiredLength);
            }
            catch when (_charBuffer != null)
            {
