    ...
    public class ApiActionsDoNotRequireExplicitModelValidationCheckAnalyzer : ApiControllerAnalyzerBase
    {
        ...
        protected override void InitializeWorker(ApiControllerAnalyzerContext apiControllerAnalyzerContext)
        {
            apiControllerAnalyzerContext.Context.RegisterSyntaxNodeAction(context =>
            {
                ...
                if (method.ReturnsVoid || method.ReturnType == apiControllerAnalyzerContext.SystemThreadingTask)
                {
                ...
                foreach (var memberAccessSyntax in methodSyntax.Body.DescendantNodes().OfType<MemberAccessExpressionSyntax>())
                {
                    ...
                    if (!(symbolInfo.Symbol is IPropertySymbol property) ||
                        (property.ContainingType != apiControllerAnalyzerContext.ModelStateDictionary) ||
                    ...
                    context.ReportDiagnostic(Diagnostic.Create(SupportedDiagnostic, ancestorIfStatement.GetLocation()));
                    return;
                }
