...
namespace Microsoft.AspNetCore.Mvc.Razor.Internal
{
    ...
    public class RazorViewCompiler : IViewCompiler
    {
        ...
        public Task<CompiledViewDescriptor> CompileAsync(string relativePath)
        {
            ...
            if (_memoryCache.TryGetValue(relativePath, out Task<CompiledViewDescriptor> cachedResult))
            {
                return cachedResult;
            }

            var normalizedPath = GetNormalizedPath(relativePath);
            if (_memoryCache.TryGetValue(normalizedPath, out cachedResult))
            {
                return cachedResult;
            }
        ...
        private Task<CompiledViewDescriptor> OnCacheMiss(string normalizedPath)
        {
            ...
            lock (_cacheLock)
            {
                ...
                if (_memoryCache.TryGetValue(normalizedPath, out Task<CompiledViewDescriptor> result))
                {
                    return result;
                }
                ...
                _memoryCache.Set(normalizedPath, taskSource.Task, cacheEntryOptions);
            }
